...
def compress_evaluate(data,size,map=[]):
    # float dhc_compress_evaluate(int16_t *data, uint32_t size, uint8_t *map);
    dhc_functions.dhc_compress_evaluate.restype = c_float
    dhc_functions.dhc_compress_evaluate.argtypes = [POINTER(c_int16),c_uint32,POINTER(c_uint8)]
    ...
    ratio = dhc_functions.dhc_compress_evaluate(c_data,c_size,c_map)
...
def test():
    ...
    ratio = compress_evaluate(input,input_size)
